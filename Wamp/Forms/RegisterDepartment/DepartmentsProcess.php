...
if(isset($_POST["Insert_HiddenID"]))
	{
		...
		if (empty($HiddenID)) {
			$resultA = $db_handle->runQuery("SELECT * FROM `departments` WHERE `Departments`='$Department'"); 
			if($resultA instanceof mysqli_result && $resultA->num_rows > 0){
				...
				$result = $db_handle->insertQuery("INSERT INTO `departments` (`Departments`) 
				Values('$Department')");
			...
			$resultA = $db_handle->runQuery("SELECT * FROM `departments` WHERE `Departments`='$Department' AND `ID` !='$HiddenID' "); 
			if($resultA instanceof mysqli_result && $resultA->num_rows > 0){
				?>
				<script>
					$(".alertWarning").fadeIn(100).html(' <span class="closebtn" onclick="this.parentElement.style.display="none";"></span>Duplicate Record!');
				...
				$result = $db_handle->updateQuery("UPDATE `departments` SET `Departments`='$Department' WHERE `ID` ='$HiddenID'");
