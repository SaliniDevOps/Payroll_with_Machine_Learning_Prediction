...
if (isset($_POST["allowanceDescription"])) {
    ...
    $resultA = $db_handle->runQuery("SELECT * FROM `varianceallowancetype` WHERE `AllowancesDescription`='$allowanceDescription'");
    if ($resultA instanceof mysqli_result && $resultA->num_rows > 0) {
        ...
        // Insert new record
        $result = $db_handle->insertQuery("INSERT INTO `varianceallowancetype` (`AllowancesDescription`) VALUES ('$allowanceDescription')");
         if ($result) {
            echo '<script>
