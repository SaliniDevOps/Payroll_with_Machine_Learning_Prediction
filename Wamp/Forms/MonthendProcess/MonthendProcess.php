<?php 
    ...
    $AccountNumber ='' ;    		$Salarytobank ='';    			$BasicSalary = '';
    ...
    $IsFemail ='';    				$isPieceEmployee ='';    		$nNopayDates ='';
    $FixedAlowAmount ='';   		$VariAllEPF ='';    			$VariaAlowAmount ='';
    $FixedDeduction ='';    		$VariaDeduction ='';    		$SalaryAdvance ='';
    $LonePayment ='';    			$SalaryArrears ='';    			$SalaArrAmount ='';
    $PayCut ='';    				$PayCutEvenin ='';    			$LoanBalance ='';
    $NonCashBenefits ='';   		$PreviousBalance ='';    		$GrossSalary ='';
    $NetSalary ='';					$TotalAllwance ='';    			$nopayAmount ='';
	...
	if(isset($_POST["Insert_cmbDepartment"]))
	{
		...
		if($payrollDon==0){
			...
			$query28 =("SELECT employee.`BasicSalary`,employee.`Insurance`,employee.`Sex`,EmpAttendance.`EMPCode`,`employee`.`InactiveEmplyee`,
						Employee.`EmployeeCode`,employee.`BalancePay`,employee.`FactoryEmploye`,employee.`TreadUnion`,employee.`DaySalary`,
						employee.`isPieceEmployee` 
						FROM EmpAttendance 
						JOIN Employee ON EmpAttendance.EMPCode = Employee.EmployeeCode 
						WHERE (EmpAttendance.Status = 1) AND (Employee.Status = 0) AND (EmpAttendance.IsPayroll = 0)
						Group BY `EmpAttendance`.`EMPCode` ASC");			
			...
			if(!empty($result)){
				foreach ($result as $r) {
					...
					$TotalAllwance = $FixedAlowAmount + $TotalVarianceAllowAmount;
					
					$GrossSalary = ($BasicSalary - $nopayAmount) + ($OTAmount + $TotalAllwance + $PreviousBalance + $SaturdayAllowence);
					$TotalDeduction =0;
					...
					$query="INSERT INTO payroll (EmployeeID,WorkingDays, NopayDays,FixedDeduction,FixedAllowance,VarianceDeduction, 
							...
							Values('$EmployeeID','$nWorkingDates','$nNopayDates','$FixedDeduction','$FixedAlowAmount ', 
							'$VariaDeduction','$TotalVarianceAllowAmount','$OTAmount',0,'$SalaryAdvance ',
