...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Retrieve form data
    $departmentID = $_POST['cmbDepartment'];
...
<body>
	...
	if(!empty($resultPayslip)) {
		...
		if($departmentID==''){
			 $query="SELECT Departments.Departments, Payroll.BasicSalary,Payroll.PoyaDayWorking, Payroll.SundayWorkingDay,Payroll.DaySalary, Employee.DepartmentID,
			 Payroll.Nopay, Payroll.OTAmount, Payroll.PieceAlowAmount, Payroll.SalaryArrears, Payroll.SunDayOtHours, Payroll.TreadUnion, Payroll.EPFEmployee,
			 Payroll.PayCut, Payroll.EPFCompany, Payroll.ETF, Payroll.NCashBenefits, Payroll.nopayAmountAllownse, Payroll.LonePayment, Payroll.NetSalary,
			 Payroll.SalaryAdvance, Payroll.LoanBalance, Payroll.GrossSalary, Payroll.EmployeeID, Payroll.SunDayOT, Employee.FirstName AS EmployeeName, 
			 Employee.LastName AS EmployeeLName,  Payroll.TempEmployee,Payroll.FixedDeduction, Payroll.FixedAllowance, Payroll.VarianceDeduction,
			 Payroll.VarianceAllowance, Payroll.BalanceBF, Payroll.BalanceCF,Payroll.SaturdayAllowence, Payroll.WorkingDays, Payroll.NormalOt, 
			 Payroll.DoubleOt, Payroll.PayCutEvenin, Payroll.BasicSalaryforEPF, Payroll.OTHours, Payroll.TotalDeduction, Payroll.OrgBasicSalery,
			 Payroll.WeekNo,Payroll.SundayAllownce FROM Employee 
			 INNER JOIN Payroll ON `employee`.`EmployeeID` = Payroll.EmployeeID 
			 INNER JOIN Departments ON Employee.DepartmentID = Departments.ID 
		...
		if(!empty($result)) {
			foreach ($result as $rowDep1) {
				...
				$EmployeeSName=$rowDep1["EmployeeName"];
				...
				$CategoryID=$rowDep1["DepartmentID"];
		...
		//		$NetSalary = $GrossSalary - ($AllTotalDeduction + $EPFEmployee + $ETF) ;//$FixedDeduction
				...
				// if($departmentID!=''){
					// $tail = "WHERE Employee.DepartmentID='$departmentID'";
				// }else{
					// $tail = "";
				// }
				// $Query_1 ="SELECT Departments.Name,FixedAllowanceType.AllowancesDescription,
						...
						// FROM Departments 
						// INNER JOIN Employee ON Departments.DepartmentID = Employee.DepartmentID
				...
				$Header1='BADR Contracting and Trading, Qatar';
				$Header2='PaySlip';
				$Header4='';
				$today = date("Y-m-d");
				...
				if($BasicSalary > 0 || $DaySalary > 0){
					...
					<table width="654" border="0" align="center">
						...
						<tr>
							...
							<td width="158" align="right"><font><?php echo number_format($BasicSalary,2); ?></font></td>
